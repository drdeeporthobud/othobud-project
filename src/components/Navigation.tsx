        ...
        <div className="flex items-center justify-between h-16 sm:h-18 py-2.5">
          ...
          <Link to="/" className="flex items-center gap-2.5 sm:gap-3 group select-none">
            ...
            <div className="min-w-0">
              <div className={`font-display font-800 text-base sm:text-lg leading-none tracking-tight transition-colors ${logoColor}`}>
                Orthobud
              </div>
              <div className={`text-[11px] sm:text-xs font-medium transition-colors truncate max-w-[140px] xs:max-w-none ${scrolled || !isHome ? 'text-navy-700' : 'text-white/70'}`}>
                Dr. Deep Chakraborty
              </div>
            </div>
          </Link>
            ...
            {navLinks.map((link) => {
              ...
              return (
                ...
                >
                  ...
                  <span className={`text-xs ${active ? 'text-teal' : 'text-white/40'}`}>→</span>
                </Link>
              )
            })}
          </div>
