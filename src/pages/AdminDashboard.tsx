  ...
  return (
    <span
      className={`inline-flex items-center gap-1 font-semibold ${
        isPediatric ? "text-rose-600" : "text-teal"
      } ${className}`}
    >
      {isPediatric ? (
...
export default function AdminDashboard() {
  ...
  const [receptionistTab, setReceptionistTab] =
    useState<"pending" | "queue" | "capacity">("queue")
  const [receptionistSubFilter, setReceptionistSubFilter] =
    useState<"all" | "waiting" | "in_consult" | "confirmed" | "completed">(
      "all",
    )
  ...
  const [declineReasonPreset, setDeclineReasonPreset] = useState<string>(
    ...
    try {
      ...
      const [list, bDates, allAppointmentsForBranch] = await Promise.all([
        ...
        appointmentService.getBlockedDates(
          clinicParam === "all" ? undefined : clinicParam,
        ),
      ...
      setAllPendingList(
        allAppointmentsForBranch.filter((a) => a.status === "pending"),
      )
    } catch (err) {
  ...
  const handleApprove = async (app: AppointmentRecord) => {
    ...
    setToastMessage(
      `Booking ${app.bookingReference} for ${app.patientName} has been approved.`,
    )
    refreshQueue()
  }
  ...
  const handleConfirmDecline = async () => {
    ...
    const finalReason = declineCustomReason.trim()
      ? declineCustomReason
      : declineReasonPreset
  ...
  const handleBlockDate = async (e: React.FormEvent) => {
    ...
    await appointmentService.blockDoctorDate(
      targetClinic,
      blockDate,
      blockReason,
    )
    setBlockModal(false)
    setToastMessage(
      `Date ${blockDate} successfully blocked. New bookings are now disabled on the public website.`,
    )
    refreshQueue()
  }

  const currentClinicObj =
  ...
  const activeBlockedDate = blockedDates.find(
    (b) =>
      b.blockedDate === filterDate &&
      (b.clinicId === currentClinicObj.id || b.clinicId === null),
  )

  // Filtering for Receptionist View
  const clinicAppointments = appointments.filter(
    (a) => a.clinicId === selectedClinicId,
  )
  const pendingAppointments = allPendingList.filter(
    (a) => a.clinicId === selectedClinicId,
  )
  const confirmedAppointments = clinicAppointments.filter(
    (a) => a.status === "confirmed",
  )
  const arrivedAppointments = clinicAppointments.filter(
    (a) => a.status === "arrived",
  )
  const inConsultAppointments = clinicAppointments.filter(
  ...
  const handleOpenWalkinModal = (preselectedSlot?: string) => {
    ...
    if (!slotToSet) {
      const firstAvailable = standardSlots.find(
        (s) =>
          !clinicAppointments.some(
            (a) => a.timeSlot === s && a.status !== "cancelled",
          ),
      )
  ...
  const handleWalkinSubmit = async (e: React.FormEvent) => {
    ...
    const isOverbookOrEmergency =
      ...
      clinicAppointments.some(
        (a) => a.timeSlot === chosenSlot && a.status !== "cancelled",
      )
    ...
    if (booked.success && booked.appointmentId) {
      ...
      setToastMessage(
        `Walk-in patient "${walkinForm.name}" registered and checked-in as Arrived.`,
      )
      refreshQueue()
    } else {
      setToastMessage(
        `Failed to register walk-in: ${booked.error || "Please try another slot."}`,
      )
    }
  }
  ...
  const queueDisplayList = filteredBySearch.filter((a) => {
    ...
    if (receptionistSubFilter === "in_consult")
    ...
    return (
      <AdminLogin
        onLoginSuccess={(newSession) => {
          ...
          if (
            newSession.role === "receptionist" &&
            newSession.assignedClinicId
          ) {
            ...
            <div className="p-4 space-y-6">
              <div>
                ...
                <nav className="space-y-1">
                  ...
                  <button className="w-full flex items-center justify-between px-3.5 py-2.5 rounded-xl text-xs font-display font-700 bg-teal/15 text-teal border border-teal/30 shadow-xs transition-all text-left cursor-default">
          ...
          <div className="bg-white rounded-3xl p-6 sm:p-7 border border-border/60 shadow-xs flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-navy-700 mt-1 max-w-2xl leading-relaxed">
                Manage doctor, receptionist, and administrator credentials,
                permissions, and security across all Orthobud clinic branches.
              </p>
            </div>
    ...
    <>
                ...
                <div className="text-left">
                  <div className="flex items-center gap-1.5">
                    <span className="text-xs font-display font-700 text-white leading-none">
                      {session.fullName === "Practice Administrator"
                        ? "Admin"
                        : session.fullName}
                    </span>
              ...
              <div className="bg-white rounded-3xl p-6 shadow-sm border border-border/60">
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 pb-6 border-b border-border/60">
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <div className="flex items-center gap-3 flex-wrap">
                        <h1 className="font-display font-800 text-xl text-navy">
                          {currentClinicObj.name}
                        </h1>
                        {activeBlockedDate ? (
                      ...
                      <p className="text-xs text-navy-700 mt-1 flex items-center gap-2 flex-wrap">
                        <span className="inline-flex items-center gap-1">
                          <MapPin className="w-3.5 h-3.5 text-navy-700/70 shrink-0" />{" "}
                          {currentClinicObj.address}
                        </span>
                        <span>•</span>
                        <span className="inline-flex items-center gap-1">
                          <Clock className="w-3.5 h-3.5 text-navy-700/70 shrink-0" />{" "}
                          {currentClinicObj.hoursDescription}
                        </span>
                        <span>•</span>
                        <span className="inline-flex items-center gap-1">
                          <Phone className="w-3.5 h-3.5 text-navy-700/70 shrink-0" />{" "}
                          {currentClinicObj.phone}
                        </span>
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-3 flex-wrap">
                    ...
                    {currentRole !== "receptionist" && (
                      <div className="flex items-center gap-1.5 bg-soft-gray p-1 rounded-xl border border-border/50">
                        <span className="text-[11px] text-navy-700 px-2 font-medium">
                          Switch Branch:
                        </span>
                        <select
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-6">
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-display font-700 text-amber-900">
                        Pending Review
                      </span>
                  ...
                  <button
                    ...
                    className={`p-4 rounded-2xl border text-left transition-all ${
                      ...
                      receptionistSubFilter === "confirmed"
                        ? "border-teal bg-teal/10 ring-2 ring-teal/20"
                        : "border-border/60 bg-soft-gray/60 hover:bg-soft-gray"
                    }`}
                  >
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-display font-700 text-navy">
                        Confirmed Today
                      </span>
                  ...
                  <button
                    ...
                    className={`p-4 rounded-2xl border text-left transition-all ${
                      receptionistTab === "queue" &&
                      receptionistSubFilter === "waiting"
                        ? "border-blue-500 bg-blue-500/10 ring-2 ring-blue-500/20"
                        : "border-border/60 bg-soft-gray/60 hover:bg-soft-gray"
                    }`}
                  >
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-display font-700 text-blue-900">
                        In Waiting Area
                      </span>
                  ...
                  <button
                    ...
                    className={`p-4 rounded-2xl border text-left transition-all ${
                      receptionistTab === "queue" &&
                      receptionistSubFilter === "completed"
                        ? "border-emerald-500 bg-emerald-500/10 ring-2 ring-emerald-500/20"
                        : "border-border/60 bg-soft-gray/60 hover:bg-soft-gray"
                    }`}
                  >
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-display font-700 text-emerald-900">
                        Completed
                      </span>
                ...
                <div className="bg-white rounded-3xl p-6 sm:p-8 shadow-sm border border-border/60">
                  <div className="flex items-center justify-between pb-4 mb-6 border-b border-border/60">
                    <div>
                      ...
                      <p className="text-xs text-navy-700 mt-0.5">
                        Review appointments requested online by patients.
                        Approving moves the slot to Confirmed and sends
                        automated WhatsApp instructions.
                      </p>
                    </div>
                    <button
                  ...
                  {loading ? (
                    ...
                    <div className="text-center py-12 bg-soft-gray/50 rounded-2xl border border-dashed border-border/80">
                      ...
                      <p className="text-sm font-display font-700 text-navy">
                        All caught up! No pending requests.
                      </p>
                      <p className="text-xs text-navy-700 mt-1">
                        New online booking requests submitted by patients will
                        pop up here in real-time.
                      </p>
                    </div>
                  ) : (
                        ...
                        >
                          <div className="space-y-2">
                            <div className="flex items-center gap-2 flex-wrap">
                              ...
                              <span className="font-display font-800 text-navy text-base">
                                {app.patientName}
                              </span>
              ...
              {receptionistTab === "queue" && (
                <div className="bg-white rounded-3xl p-6 sm:p-8 shadow-sm border border-border/60">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 mb-6 border-b border-border/60">
                    <div>
                      <h2 className="font-display font-800 text-lg text-navy flex items-center gap-2">
                        ...
                        <span className="text-xs font-normal text-navy-700">
                          ({filterDate})
                        </span>
                      </h2>
                      <p className="text-xs text-navy-700 mt-0.5">
                        Mark patients as <strong>Arrived</strong> when they
                        reach the clinic desk so Dr. Deep sees them in the
                        consultation room.
                      </p>
                    </div>
                    ...
                    <div className="flex items-center gap-1.5 bg-soft-gray p-1 rounded-xl text-xs flex-wrap">
                      {[
                        ...
                        <button
                          key={f.id}
                          onClick={() =>
                            setReceptionistSubFilter(
                              f.id as typeof receptionistSubFilter,
                            )
                          }
                  ...
                  {loading ? (
                    ...
                    <div className="text-center py-12 bg-soft-gray/50 rounded-2xl border border-dashed border-border/80">
                      <p className="text-sm font-display font-700 text-navy">
                        No patients in this queue for {filterDate}.
                      </p>
                      <p className="text-xs text-navy-700 mt-1">
                        Use <strong>"+ Quick Walk-In"</strong> to check-in an
                        arriving patient directly.
                      </p>
                    </div>
                  ) : (
                        ...
                        >
                          <div className="flex items-start gap-4">
                            ...
                            <div className="w-12 h-12 rounded-2xl bg-teal/10 text-teal flex flex-col items-center justify-center font-display font-800 text-sm shrink-0 border border-teal/20">
                              <span className="text-[9px] uppercase tracking-wider text-navy-700">
                                Token
                              </span>
                              <span>
                                #{stableTokenMap.get(app.id) ?? idx + 1}
                              </span>
                            </div>

                            <div>
                              <div className="flex items-center gap-2 flex-wrap">
                                <span className="font-display font-700 text-navy text-base">
                                  {app.patientName}
                                </span>
                                <span className="text-xs text-navy-700 font-medium">
                                  ({app.patientAge}y · {app.patientGender})
                            ...
                            {app.status === "confirmed" && (
                              <button
                                onClick={() => {
                                  ...
                                  setToastMessage(
                                    `Patient ${app.patientName} marked as Arrived at desk!`,
                                  )
                                }}
                            ...
                            {(app.status === "confirmed" ||
                              ...
                              <button
                                onClick={() => {
                                  if (
                                    confirm(
                                      `Mark ${app.patientName} as No-Show?`,
                                    )
                                  ) {
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 mb-6 border-b border-border/60">
                    <div>
                      <h2 className="font-display font-800 text-lg text-navy flex items-center gap-2">
                        ...
                        <span>
                          Slot Capacity Matrix — {currentClinicObj.name}
                        </span>
                      </h2>
                      <p className="text-xs text-navy-700 mt-0.5">
                        Session Schedule for {filterDate} ({selectedDayName}) ·
                        30-Minute Consultation Windows
                      </p>
                    </div>
                  ...
                  {activeBlockedDate ? (
                      ...
                      <p className="text-xs text-amber-800 mt-2 max-w-lg mx-auto font-medium">
                        ...
                        <strong>
                          {activeBlockedDate.reason ||
                            "Emergency Hospital Surgery / Medical Conference"}
                        </strong>
                      </p>
                      <p className="text-xs text-amber-700/80 mt-1 max-w-md mx-auto">
                        All consultation slots for {currentClinicObj.name} are
                        blocked for this date. Online patient bookings are
                    ...
                    <div className="bg-slate-50 border-2 border-dashed border-slate-300 rounded-3xl p-8 sm:p-12 text-center my-4">
                      ...
                      <p className="text-xs text-navy-700 mt-2 max-w-lg mx-auto leading-relaxed">
                        Dr. Deep Chakraborty does not hold OPD consultations at
                        this clinic branch on {selectedDayName}s.
                      </p>
                      <div className="inline-flex items-center gap-1.5 mt-4 px-4 py-2 bg-white rounded-xl border border-border text-xs font-semibold text-teal shadow-xs">
                        ...
                        <span>
                          Operating Days: {operatingDayNames} ·{" "}
                          {currentClinicObj.hoursDescription}
                        </span>
                      </div>
                      <p className="text-[11px] text-navy-700/70 mt-3">
                        Please select an active operating day from the date
                        selector above to view or assign slots.
                      </p>
                    </div>
                  ) : (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                      {standardSlots.map((slot) => {
                        const bookedPatients = clinicAppointments.filter(
                          (a) =>
                            a.timeSlot === slot && a.status !== "cancelled",
                        )
                        const isOccupied = bookedPatients.length > 0
                        const hasPending = bookedPatients.some(
                          (a) => a.status === "pending",
                        )
                            ...
                            {isOccupied ? (
                              <div className="mt-3 pt-3 border-t border-border/50 space-y-2">
                                {bookedPatients.map((p) => (
                                  <div
                                    key={p.id}
                                    className="text-xs bg-white p-2.5 rounded-xl border border-border/60"
                                  >
                                    <div className="flex items-center justify-between">
                                      <div className="font-display font-700 text-navy">
                                        {p.patientName}
                                      </div>
                                      <span
              ...
              <div className="bg-white rounded-3xl p-6 sm:p-8 shadow-sm border border-border/50">
                <div className="flex items-center justify-between mb-6 pb-4 border-b border-border/60">
                  <div>
                    ...
                    <p className="text-xs text-navy-700 mt-0.5">
                      Showing scheduled patients for {filterDate} · Total
                      Bookings: <strong>{appointments.length}</strong>
                    </p>
                  </div>
                  <button
                    onClick={refreshQueue}
                ...
                {loading ? (
                  <div className="text-center py-12">
                    <div className="w-8 h-8 border-3 border-teal border-t-transparent rounded-full animate-spin mx-auto mb-2" />
                    <p className="text-xs text-navy-700">
                      Updating patient queue...
                    </p>
                  </div>
                ) : appointments.length === 0 ? (
                  <div className="text-center py-12 bg-soft-gray rounded-2xl">
                    <p className="text-sm font-display font-600 text-navy">
                      No appointments scheduled for this date.
                    </p>
                    <p className="text-xs text-navy-700 mt-1">
                      Patients booking on the public website will appear here
                      automatically.
                    </p>
                  </div>
                ) : (
                    ...
                    {appointments.map((app, idx) => (
                      ...
                      >
                        <div className="flex items-start gap-4">
                          ...
                          <div>
                            <div className="flex items-center gap-2 flex-wrap">
                              <span className="font-display font-700 text-navy text-sm">
                                {app.patientName}
                              </span>
                              <span className="text-xs text-navy-700 font-medium">
                                ({app.patientAge}y · {app.patientGender})
        ...
        {declineModalOpen && (
          <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-xs flex items-center justify-center p-4">
            <div className="bg-white rounded-3xl max-w-md w-full p-6 sm:p-8 shadow-2xl border border-border/50 animate-fade-up">
              ...
              <p className="text-xs text-navy-700 mt-1">
                Select a reason for declining this request. This will be logged
                in clinic records.
              </p>
        ...
        {activeNoteAppId && (
          <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-xs flex items-center justify-center p-4">
            <div className="bg-white rounded-3xl max-w-lg w-full p-6 sm:p-8 shadow-2xl border border-border/50 animate-fade-up">
              ...
              <p className="text-xs text-navy-700 mt-1">
                Record diagnosis, recommended physical therapy, X-ray remarks,
          ...
          <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-xs flex items-center justify-center p-4">
            <div className="bg-white rounded-3xl max-w-md w-full p-6 sm:p-8 shadow-2xl border border-border/50 animate-fade-up">
              ...
              <p className="text-xs text-navy-700 mt-1 leading-relaxed">
                When a date is blocked, all public slots for that date become
        ...
        {walkinModal && (
          <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-xs flex items-center justify-center p-4">
            <div className="bg-white rounded-3xl max-w-md w-full p-6 sm:p-8 shadow-2xl border border-border/50 animate-fade-up">
              ...
              <p className="text-xs text-navy-700 mt-1">
                Add a patient to today's queue at{" "}
                <strong>{currentClinicObj.name}</strong>.
              </p>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setWalkinForm({ ...walkinForm, gender: e.target.value })
                      }
                      className="w-full border border-border/80 rounded-xl p-2.5 text-xs text-navy focus:outline-none focus:border-teal bg-white"
                    >
                  ...
                  <div>
                    ...
                    >
                      ...
                      <option value="Emergency Fit-In (Now)">
                        [EMERGENCY] Immediate Emergency / Fit-In
                      </option>
                    </select>
                  </div>
                </div>
          ...
          <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-xs flex items-center justify-center p-4">
            <div className="bg-white rounded-3xl max-w-sm w-full p-6 text-center shadow-2xl border border-border/50 animate-fade-up">
              ...
              <h3 className="font-display font-800 text-lg text-navy">
                Broadcast Clinic Delay
              </h3>
              <p className="text-xs text-navy-700 mt-1">
                Notify patients in the waiting queue regarding consultation
                schedule delays.
              </p>
              ...
              <div className="flex gap-3 mt-6">
                ...
                <button
                  ...
                  onClick={() => {
                    setDelayModal(false)
                    alert(
                      `Delay alert (${delayMinutes} mins) broadcasted to receptionist desk!`,
                    )
                  }}
      ...
      <div className="hidden print:block p-8 bg-white text-black min-h-screen font-sans">
          ...
          <div className="flex justify-between items-start">
            <div>
              ...
              <p className="text-xs text-black/80 font-medium">
                MS (Ortho), DNB (Ortho), MNAMS · Consultant Orthopedic & Joint
                Replacement Surgeon
              </p>
              <p className="text-xs text-black/70 mt-1">
                <strong>{currentClinicObj.name}</strong> ·{" "}
                {currentClinicObj.address} · Phone: {currentClinicObj.phone}
              </p>
            </div>
            <div className="text-right">
              ...
              <p className="text-sm font-bold text-black mt-2 font-mono">
                Date: {filterDate} ({selectedDayName})
              </p>
              <p className="text-xs text-black/70 font-medium">
                Total Scheduled: {stableDayAppointments.length}
              </p>
            </div>
          </div>
        </div>
        ...
        {stableDayAppointments.length === 0 ? (
          <div className="text-center py-12 border border-dashed border-gray-400 rounded-lg">
            <p className="text-sm italic text-gray-700">
              No scheduled patients on record for {filterDate}.
            </p>
          </div>
        ) : (
          <table className="w-full text-left text-[11px] border-collapse border border-black">
            <thead>
              <tr className="bg-gray-100 border-b border-black">
                <th className="p-2 border-r border-black font-bold w-12 text-center">
                  Token
                </th>
                <th className="p-2 border-r border-black font-bold w-20">
                  Time
                </th>
                <th className="p-2 border-r border-black font-bold">
                  Patient Name
                </th>
                <th className="p-2 border-r border-black font-bold w-16 text-center">
                  Age/Sex
                </th>
                <th className="p-2 border-r border-black font-bold w-28">
                  Phone
                </th>
                <th className="p-2 border-r border-black font-bold">
                  Chief Complaint
                </th>
                <th className="p-2 border-r border-black font-bold w-24">
                  Status
                </th>
            ...
            <tbody>
              {stableDayAppointments.map((app) => (
                <tr key={app.id} className="border-b border-black/30">
                  ...
                  <td className="p-2 border-r border-black/30 font-medium">
                    {app.timeSlot}
                  </td>
                  <td className="p-2 border-r border-black/30 font-bold">
                    {app.patientName}
                    {app.firstVisit && (
                      <span className="text-[9px] font-normal italic ml-1">
                        (New)
                      </span>
                    )}
                  </td>
                  ...
                  <td className="p-2 border-r border-black/30 font-mono">
                    {app.patientPhone}
                  </td>
        ...
        <div className="mt-8 pt-4 border-t border-black/40 flex justify-between text-[11px] text-black/70">
          <span>
            Printed from Orthobud Practice Internal Portal ·{" "}
            {new Date().toLocaleDateString()}
          </span>
          <span>
            Consultant Surgeon's Signature: ___________________________
          </span>
        </div>
      </div>
    </>
