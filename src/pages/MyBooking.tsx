...
export default function MyBooking() {
  ...
  const handleCancel = async () => {
    ...
    try {
      ...
      if (res.success) {
        setAppointment((prev) =>
          prev
            ? { ...prev, status: "cancelled", cancellationReason: cancelReason }
            : null,
        )
        setCancelModal(false)
        setCancelReason("")
        setFeedbackMsg(
          "Your appointment has been cancelled. The time slot has been freed up.",
        )
      } else {
  ...
  const statusColors: Record<AppointmentRecord["status"], {
    bg: string
    text: string
    label: string
  }> = {
      ...
      <section className="bg-navy py-10 sm:py-14">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 text-center">
          ...
          <p className="text-white/70 mt-2 text-xs xs:text-sm max-w-md mx-auto leading-relaxed px-2">
            Enter your 6-digit Booking Reference or registered Phone Number to
            view, reschedule, or manage your consultation.
          </p>

          {/* Search Box */}
          <form
            onSubmit={handleSearch}
            className="max-w-md mx-auto mt-6 sm:mt-7 flex flex-col sm:flex-row gap-2.5 sm:gap-2"
          >
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 mt-6 sm:mt-8">
        ...
        {!loading && appointment && (
          <div className="bg-white rounded-2xl sm:rounded-3xl p-5 xs:p-6 sm:p-9 shadow-sm border border-border/50 animate-fade-up space-y-5 sm:space-y-6">
            ...
            <div className="flex flex-col xs:flex-row xs:items-center justify-between gap-3 border-b border-border/60 pb-4 sm:pb-5">
              <div>
                <span className="text-[11px] sm:text-xs text-navy-700 block font-medium">
                  Booking Reference
                </span>
                <span className="font-display font-800 text-lg sm:text-xl text-navy tracking-tight">
                  {appointment.bookingReference}
                </span>
              </div>
              <div
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 text-xs sm:text-sm">
              <div className="bg-soft-gray rounded-xl p-3.5 sm:p-4">
                <span className="text-navy-700 block text-[11px] sm:text-xs mb-0.5">
                  Patient Name
                </span>
            ...
            {appointment.doctorClinicalNotes && (
              <div className="bg-purple-500/10 border border-purple-500/30 rounded-xl p-3.5 sm:p-4 text-xs">
                <strong className="text-purple-900 block mb-1 font-semibold">
            ...
            {appointment.status === "confirmed" && (
              <div className="bg-teal/8 border border-teal/20 rounded-xl p-3.5 sm:p-4 text-xs text-navy-700 space-y-1.5 leading-relaxed">
                <strong className="text-navy block font-display font-700">
                ...
                <p>
                  2. Bring prior X-rays, MRI scans, previous orthopedic
                  prescriptions, and valid ID.
                </p>
              </div>
            )}
            ...
            {appointment.status !== "cancelled" && (
              <div className="pt-2 border-t border-border/60 space-y-2.5 sm:space-y-3">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 sm:gap-3">
                  ...
                  >
                    <img
                      src="/icons/svg/calendar-plus.svg"
                      alt="Add to Calendar"
                  ...
                  >
                    <img
                      src="/icons/svg/google-calendar.svg"
                      alt="Google Calendar"
        ...
        {!loading && searched && !appointment && (
          <div className="bg-white rounded-2xl sm:rounded-3xl p-6 xs:p-8 sm:p-10 shadow-sm border border-border/50 text-center animate-fade-up">
            ...
            <h3 className="font-display font-700 text-navy text-lg sm:text-xl">
              No Booking Found
            </h3>
            <p className="text-navy-700 text-xs sm:text-sm mt-2 max-w-sm mx-auto leading-relaxed">
              We couldn't find an appointment matching "
              <strong>{searchQuery}</strong>". Please verify your booking
              reference or phone number.
            </p>
            <div className="mt-6 flex justify-center">
              <Link
                to="/book-appointment"
                className="btn-primary text-xs sm:text-sm px-6 py-3 w-full sm:w-auto"
              >
