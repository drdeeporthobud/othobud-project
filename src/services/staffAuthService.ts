...
export const staffAuthService = {
  ...
  async getSession(): Promise<StaffSession | null> {
    try {
      ...
      const { data: profile, error } = await sb
        .from("user_profiles")
        .select(
          "id, username, email, full_name, role, assigned_clinic_id, phone, is_active",
        )
  ...
  async login(usernameInput: string, passwordInput: string): Promise<{
    ...
    try {
      ...
      if (!profile) {
        console.error(
          "[staffAuthService] Profile not found for user ID:",
          user.id,
        )
        await sb.auth.signOut()
        return {
  ...
  async getStaffUsers(): Promise<StaffUser[]> {
    ...
    const { data, error } = await sb
      .from("user_profiles")
      .select(
        "id, username, email, full_name, role, assigned_clinic_id, phone, is_active, created_at",
      )
    ...
    const { data: clinicsData } = await sb.from("clinics").select("id, name")

    const clinicMap = new Map(
      (clinicsData || []).map((c: any) => [c.id, c.name]),
    )
    ...
    return (data || []).map((item: any) => ({
      ...
      assignedClinicName: item.assigned_clinic_id
        ? clinicMap.get(item.assigned_clinic_id) || null
        : null,
      phone: item.phone,
  ...
  async createStaffUser(params: {
    ...
    if (params.role === "receptionist" && !params.clinicId) {
      return {
        success: false,
        error: "A clinic branch must be assigned for receptionists.",
      }
    }
   ...
   * Requires Supabase — no local fallback.
   */
  async resetStaffPassword(userId: string, newPassword: string): Promise<{
    success: boolean
    error?: string
  }> {
   ...
   * Requires Supabase — no local fallback.
   */
  async toggleStaffStatus(userId: string, isActive: boolean): Promise<{
    success: boolean
    error?: string
  }> {
    const sb = requireSupabase()
   ...
   * Calls Supabase RPC — no local fallback.
   */
  async deleteStaffUser(userId: string): Promise<{
    success: boolean
    error?: string
  }> {
    const sb = requireSupabase()
